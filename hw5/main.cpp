    ...
    for (int i = 0; i < int(original.size()); i++) {
        ...
        for (int k = 0; k < int(right.size()); k++) {
            if (original[i] == right[k]) {
                assert (found == false);
                found = true;
                num_shifts += std::abs(i-(int(left.size())+k));
...
/*
    ...
    int s_e1 = 0; 
    int s_o1 = 0;
    computer_separate(num_engines, num_total, s_e1, s_o1);
    std::cout << "Engines" << s_e1 << std::endl;
    std::cout << "Other" << s_o1 << std::endl;
    */
}
