...
class Offset {
    public:
        Offset(unsigned char dx_=0, unsigned char dy_=0, int total_=0) : dx(dx_),dy(dy_),total(total_) {}
        unsigned char dx, dy;
        int total;
};

//bool more_total(const Offset & os1, const Offset & os2);

// Create a Node class to help store information of i, j and total
class Node {
    public:
        Node(int i_ = 0, int j_ = 0, int total_ = 0) : i(i_), j(j_), total(total_) {}
        int i;
        int j;
        int total;
};

bool more_total(const Node & nd1, const Node & nd2);
