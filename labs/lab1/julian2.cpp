...
int main()
{
    ...
    int Day[num];
    int Julian[num];
    // READ the month/day combination
    for(int i = 0; i < num; ++ i){
        cout << "Please enter the " << i + 1 << " month/day." << endl;
        cin >> month >> day;
        Month[i] = month;
        Day[i] = day;
    }
    // Give the julian array with values
    for(int i = 0; i < num; ++ i){
        Julian[i] = julian_day(Month[i], Day[i], year);
    }
    // Output all the julian array value
    for(int i = 0; i < num; ++ i){
        cout << " The " << Month[i] << "/" << Day[i] << " julian day is: " << Julian[i] << endl;
    }
