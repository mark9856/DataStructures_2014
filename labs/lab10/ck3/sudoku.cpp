...
Sudoku::Sudoku(int s) {
    ...
    for(int i = 0; i < s; ++ i){
        board.push_back(vector<set<int> >());
        for(int j = 0; j < s; ++ j){
            //board[i][j].clear();
            board[i].push_back(set<int> ());
            for(int k = 1; k < s + 1; ++ k){
                board[i][j].insert(k);
            }
        }
...
bool Sudoku::IsSolved() const {
    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if(board[i][j].size() > 1) solved = false;
        }
    }
...
int Sudoku::Propagate(int i, int j) {
    ...
    set<int>::iterator p = board[i][j].begin();
    int value = *p;
    for(int k = 0; k < size; ++ k){
        if(board[i][k].size() != 1){
            //if (board[i][k].find(value) != board[i][j].end()){
                board[i][k].erase(value);
            //}
        }
    }
    for(int r = 0; r < size; ++ r){
        if(board[r][j].size() != 1){
            //if (board[r][j].find(value) != board[r][j].end()){
                board[r][j].erase(value);
            //}
        }
    }
