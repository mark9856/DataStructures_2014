...
class ds_set {
    ...
    private:
        ...
        TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
            ...
            this -> root_ = new TreeNode<T>(old_root -> value);

            copyToTree(this -> root_, old_root);

            return this -> root_;
        }

        void copyToTree(TreeNode<T>* new_root, TreeNode<T>* old_root){
            std::pair< ds_set<std::string>::iterator, bool > insert_result;
            if (old_root) {
                insert_result = insert(old_root -> value, new_root);
                //assert(insert_result.second);
                copyToTree(new_root, old_root -> left);


                copyToTree(new_root, old_root -> right);
            }
        }
