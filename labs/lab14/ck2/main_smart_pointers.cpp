...
/*
void deleteA(const Balloon* b, const Balloon* constB){
    if(b -> numRopes() == 0){
        delete b;
        return;
    }
    else if(b -> numRopes() == 1 && b -> getRope(0) == constB){
        delete b;
        return;
    }
    else{
        for(int i = 0; i < b -> numRopes(); ++ i){
            if(b -> getRope(i) -> numRopes() != 0){
                deleteA(b -> getRope(i), constB);
            }
            delete b -> getRope(i);
        }
        delete b;
...
/*
void helpDelete(const Balloon * b, Balloon * constB){
    if(b -> numRopes() == 0){
        delete b;
        return;
    }
...
/*
void helpDelete(Balloon* &b, std::vector<Balloon*> &balloons){
    if(b -> numRopes() == 0){
        delete b;
        return;
    }
        ...
        if(!found){
            ...
            for(int i = 0; i < b -> numRopes(); ++ i){
                if(b -> getRope(i) -> numRopes() != 0){
                    helpDelete(b -> getRope(i), balloons);
                }
                delete b -> getRope(i);
            }
            delete b;
...
*/
    ...
    for(int i = 0; i < georgette -> numRopes(); ++ i){
        delete georgette -> getRope(i);
    }
    delete georgette;
*/
    /*
    ...
    */
    ...
    /*
    dsSharedPtr<Balloon> georgette1(new Balloon("Mr Potato Head"));
    dsSharedPtr<Balloon> henry1(new Balloon("Snoopy"));

    georgette1->addRope(henry1);
    henry1 = new Balloon("Tigger");
    georgette1->addRope(henry1);
    georgette1->print();
    henry1->print();

    dsSharedPtr<Balloon> isabelle1(new Balloon("Shrek"));
    henry1->addRope(isabelle1);
    isabelle1 = new Balloon("Barney the Purple Dinosaur");
    georgette1->addRope(isabelle1);
