...
template <class T> typename Vec<T>::size_type remove_matching_elements(Vec<T> &v, T &t){
    /*
    typename Vec<T>::size_type cout = 0;
    ...
    return cout;
    */
    // set up the count number for the number of elements that are the same
    typename Vec<T>::size_type count = 0;
    // Start from begin of the Vec till the end
    for (typename Vec<T>::iterator p = v.begin(); p < v.begin() + v.size(); ++ p){
        // when the element is the same
        if(*p == t){
            // increase the count
            count ++;
            // erase the same element from the Vec
            v.erase(p);
            // lead the pointer back to next element
            p --;
        }
    }
    // when there is no the same element
    if(count == 0){
        std::cout << "no element to be remove" << std::endl;
    }
    // resize the Vec, make sure the m_alloc is not too big
    v.Vec<T>::resize(v.size());
    
    return count;
}

template <class T> void Vec<T>::print(){
    std::cout << "m_alloc is " << m_alloc << std::endl;
    std::cout << "m_size is " << m_size << std::endl;
    std::cout << "m_data is " << std::endl;
    for(iterator p = m_data; p < m_data + m_size; ++ p){
        std::cout << *p << " ";
    }
}
...
template <class T> const Vec<T>& Vec<T>::operator=(const Vec<T>& v) {
    if (this != &v) {
        delete [] m_data;
        this -> copy(v);
    }
