...
void dslist<T>::push_back(const T& v) {
    ...
    if (!tail_) {
        Node<T>* finalNode = new Node<T>( v );
        head_ = tail_ = newp;
        newp -> next_ = finalNode;
        finalNode -> prev_ = newp;
    }
    // normal case:  at least one node already
    else {
        Node<T>* finalNode;
        finalNode = tail_ -> next_;
        tail_ -> next_ = newp;
        newp -> prev_ = tail_;
        finalNode -> prev_ = newp;
        newp -> next_ = finalNode;
        tail_ = newp;
        /*
        tail_ -> next_ -> prev_ = newp;
        newp -> prev_ = tail_;
        newp -> next_ = tail_ -> next_;
        tail_ -> next_ = newp;
        tail_ = newp;
        */
        /*
        ...
        tail_ -> next_ = newp;
        tail_ = newp;
        tail_ -> next_ = finalNode;
        finalNode -> prev_ = tail_;
        */
    }
    ++size_;
}
